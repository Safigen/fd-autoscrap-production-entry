var N = Math.log(2);
function i(v, n, h, l, m, u) {
  var a = l + "-" + m, f = v.length;
  if (u.hasOwnProperty(a))
    return u[a];
  if (n === 1) {
    var g = Math.round(Math.log((1 << f) - 1 & ~m) / N);
    return v[h][g];
  }
  for (var R = l | 1 << h, b = h + 1; l & 1 << b; )
    b++;
  for (var p = 0, y = 0, A = 0; y < f; y++) {
    var K = 1 << y;
    K & m || (p += (A % 2 ? -1 : 1) * v[h][y] * i(v, n - 1, b, R, m | K, u), A++);
  }
  return u[a] = p, p;
}
function O(v, n) {
  ...
  if (m !== 0) {
    for (var u = [], a = 0; a < 8; a++)
      for (var f = 0; f < 8; f++)
        u[f] == null && (u[f] = 0), u[f] += ((a + f) % 2 ? -1 : 1) * i(h, 7, a === 0 ? 1 : 0, 1 << a, 1 << f, l) / m * n[a];
...
export {
  O as buildTransformer
};
//# sourceMappingURL=fourPointsTransform.js.map
